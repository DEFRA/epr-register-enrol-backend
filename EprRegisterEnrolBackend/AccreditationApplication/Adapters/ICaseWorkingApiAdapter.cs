...
public interface ICaseWorkingApiAdapter
{
    ...
    // log; SlaDueDate (RA-415) comes straight off the same work item response — the Case Management service's SLA due
    // date is the Registration & Accreditation service's single source of truth for "Due date", so no separate lookup is needed.
