...
// RA-311 fix 3: the "DefaultClient" HttpClient is used for the OJ BE -> ManagementBe submit
// call (HttpCaseWorkingApiAdapter.SubmitApplicationAsync). Without an explicit Timeout it
// defaults to .NET's 100s, which can leave OJ BE still waiting long after OJ FE's own submit
// budget has given up. This asserts the registration in Program.cs actually wires up the
// intended 15s timeout, rather than relying only on the adapter-level behavioural tests.
